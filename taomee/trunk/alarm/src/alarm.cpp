...
int alarm_p_metric_data(DEFAULT_ARG)
{
    ...
    if (p_in->fail_flag)
    {
        if (2 == p_alarm->m_fail || 0 == p_alarm->m_fail)
        {
            ...
            // else if (0 == strncmp(p_node->m_node_desc, "Idle", PROJECT_NAME_LEN))
            // {
                // // 空闲的机器不发采集失败告警
            // }
...
int handle_mysql_collect_fail(c_metric_alarm * p_alarm)
{
    ...
    {
        ERROR_LOG("instance alive collect failed, node[%u: %s]", 
                p_node->m_node_id, long2ip(p_node->m_node_ip));
        send_collect_fail_alarm(p_alarm);
        return 0;
    }
    ...
    {
        ERROR_LOG("no instance alive metric found, node[%u: %s], metric[%u: %s]", 
                p_node->m_node_id, long2ip(p_node->m_node_ip),
    ...
    {
        ERROR_LOG("no instance alive metric alarm found, node[%u: %s], metric[%u: %s](%s)", 
                p_node->m_node_id, long2ip(p_node->m_node_ip),
                p_metric->m_metric_id, p_metric->m_metric_name, p_alarm->m_arg);
        send_collect_fail_alarm(p_alarm);
        return 0;
    ...
    if (0 == p_instance_alarm->m_fail)
    {
        ...
        ERROR_LOG("instance alive collect fail = 0, node[%u: %s]", 
                p_node->m_node_id, long2ip(p_node->m_node_ip));
        p_alarm->m_fail = 0;
        return 0;
    }
    ...
    {
        ERROR_LOG("instance alive collect fail = 1, node[%u: %s]", 
                p_node->m_node_id, long2ip(p_node->m_node_ip));
    ...
    else if (2 == p_instance_alarm->m_fail)
    {
        ...
        {
            ERROR_LOG("instance alive collect value = 2, node[%u: %s]", 
                    p_node->m_node_id, long2ip(p_node->m_node_ip));
        ...
        {
            ERROR_LOG("instance alive collect value = 1, node[%u: %s]", 
                    p_node->m_node_id, long2ip(p_node->m_node_ip));
...
int send_metric_alarm(const c_metric_alarm * p_alarm)
{
    ...
    if (way & ALARM_WAY_EMAIL || way & ALARM_WAY_RTX)
    {
        snprintf(g_msg_buf, sizeof(g_msg_buf),
                "%s:%s(%s) %s(%s) current(%s) %s threshold(%s) %s",
                flag, 
                node_ip, 
                p_node->m_node_desc,
                p_metric->m_metric_name,
...
int send_node_alarm(c_node * p_node)
{
    ...
    { 
        ...
        snprintf(g_msg_buf, sizeof(g_msg_buf), "%s(%s) itl node 恢复 %s",
                long2ip(p_node->m_node_ip),
                p_node->m_node_desc,
                ts);
    }
    ...
    send_rtx_alarm(config_get_strval("itl_node_contact", "tongyliu,smyang,elvacheng"), g_msg_buf);
