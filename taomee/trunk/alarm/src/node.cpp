...
c_node * alloc_node(uint32_t node_id, int fd)
{
    ...
    p_node->m_head_fd = fd;
    memset(p_node->m_node_desc, 0, sizeof(p_node->m_node_desc));
...
int free_node(c_node * p_node, void * param)
{
    ...
    {
        REMOVE_TIMER(p_node->m_node_timer);
    }

    if (NULL != p_node->m_host_timer)
    {
        REMOVE_TIMER(p_node->m_host_timer);
    }
...
int init_server_node(c_node * p_node, Cmessage * c_out)
{
    ...
    STRNCPY(p_node->m_node_desc, p_out->project_name, PROJECT_NAME_LEN);
    ...
    DEBUG_LOG("\tnode[%u: %s], project: %s", p_node->m_node_id, long2ip(p_node->m_node_ip), p_node->m_node_desc);
...
int init_mysql_node(c_node * p_node, Cmessage * c_out)
{
    ...
    DEBUG_LOG("\tnode id: %u, project: %s", p_node->m_node_id, p_node->m_node_desc);
