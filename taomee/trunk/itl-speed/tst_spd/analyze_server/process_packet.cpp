		...
		if (cdn_speed_ip_mmap[i].mmap_head->ready_num != 0) {
			...
			if (fwrite(cdn_speed_ip_mmap[i].mmap_head, 1, cdn_mmap_size_ip + MMAP_FILE_HEAD_LEN, fp)
				...
				for (j = 0; j < cdn_speed_ip_mmap[i].mmap_head->records_num; j++) {
					...
					(ptr + j)->step = pos;
					memset((ptr + j)->speed_distribution, 0, sizeof((ptr + j)->speed_distribution));
				}
			}
		}
...
int set_cdn_mmap(char *buf, uint32_t data_len)
{
	...
	if (offset == NULL) {
		...
		int k;
        for ( k = 0; k < CDN_TIME_INTER; k++) {
            cdn_ip_p->speed[(time_step + k) % CDN_TIME_INTER] = (speed_invalid == false) ?packet->record.m_cdn.speed :0;
            cdn_ip_p->count[(time_step + k) % CDN_TIME_INTER] = 1;//(speed_invlaid == false) ?1 :0;
        }
		//added by singku for speed distribution 2011-11-21
		uint32_t tmp_speed = packet->record.m_cdn.speed / (1024); //KB
		if (speed_invalid) {
			cdn_ip_p->speed_distribution[0] = 1;
		} else {
			if (tmp_speed < cdn_speed_dis[0]) {
				cdn_ip_p->speed_distribution[1] = 1;
			} else if (tmp_speed < cdn_speed_dis[1]) {
				cdn_ip_p->speed_distribution[2] = 1;
			} else if (tmp_speed < cdn_speed_dis[2]) {
				cdn_ip_p->speed_distribution[3] = 1;
			} else if (tmp_speed < cdn_speed_dis[3]) {
				cdn_ip_p->speed_distribution[4] = 1;
			} else {
				cdn_ip_p->speed_distribution[5] = 1;
			}
		}
		//added by singku for speed distribution 2011-11-21
		...
		for ( k = 0; k < CDN_TIME_INTER; k++) {
			cdn_ip_p->speed[(time_step + k) % CDN_TIME_INTER] += (speed_invalid == false) ?packet->record.m_cdn.speed :0;
            if (cdn_ip_p->count[(time_step + k) % CDN_TIME_INTER] == 0) {
                cdn_ip_p->count[(time_step + k) % CDN_TIME_INTER] = 1;
            } else {
			    cdn_ip_p->count[(time_step + k) % CDN_TIME_INTER] += (speed_invalid == false) ?1 :0;
		    }
        }
		//added by singku for speed distribution 2011-11-21
		uint32_t tmp_speed = packet->record.m_cdn.speed / (1024);
		if (speed_invalid) {
			cdn_ip_p->speed_distribution[0] ++;
		} else {
			if (tmp_speed < cdn_speed_dis[0]) {
				cdn_ip_p->speed_distribution[1] ++;
			} else if (tmp_speed < cdn_speed_dis[1]) {
				cdn_ip_p->speed_distribution[2] ++;
			} else if (tmp_speed < cdn_speed_dis[2]) {
				cdn_ip_p->speed_distribution[3] ++;
			} else if (tmp_speed < cdn_speed_dis[3]) {
				cdn_ip_p->speed_distribution[4] ++;
			} else {
				cdn_ip_p->speed_distribution[5] ++;
			}
		}
		//added by singku for speed distribution 2011-11-21
	}
	...
	if (offset == NULL)
		...
		int k;
		for ( k = 0; k < CDN_TIME_INTER; k++) {
			cdn_area_p->speed[(time_step + k) % CDN_TIME_INTER] += (speed_invalid == false) ?packet->record.m_cdn.speed :0;
            if (cdn_area_p->count[(time_step + k) % CDN_TIME_INTER] == 0) {
                cdn_area_p->count[(time_step + k) % CDN_TIME_INTER] = 1;
            } else {
			    cdn_area_p->count[(time_step + k) % CDN_TIME_INTER] += (speed_invalid == false) ?1 :0;
		    }
        }
		...
		uint32_t tmp_speed = packet->record.m_cdn.speed / (1024);
		if (speed_invalid) {
			cdn_area_p->speed_distribution[0] ++;
		} else {
			if (tmp_speed < cdn_speed_dis[0]) {
				cdn_area_p->speed_distribution[1] ++;
			} else if (tmp_speed < cdn_speed_dis[1]) {
				cdn_area_p->speed_distribution[2] ++;
			} else if (tmp_speed < cdn_speed_dis[2]) {
				cdn_area_p->speed_distribution[3] ++;
			} else if (tmp_speed < cdn_speed_dis[3]) {
				cdn_area_p->speed_distribution[4] ++;
			} else {
				cdn_area_p->speed_distribution[5] ++;
			}
		}
		//added by singku for speed distribution 2011-11-21
	}
