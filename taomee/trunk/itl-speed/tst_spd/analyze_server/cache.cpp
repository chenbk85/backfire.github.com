...
int update_cache(uint16_t type, void *data)
{
	...
	case proto_cache_cdn_area: {
			...
			if (src_cdn_area->t_time/60 > dest_cdn_area->data[time_step].test_time/60) {
				...
				int m;
				for (m = 0; m < 6; m++) {
					dest_cdn_area->speed_distribution[time_step][m] += src_cdn_area->speed_distribution[m];
				}
			}
		}
		break;
	case proto_cache_cdn_node: {
			...
			if (src_cdn_ip->t_time/60 > dest_cdn_node->data[time_step].test_time/60) {
				...
				memcpy(dest_cdn_node->speed_distribution[time_step], src_cdn_ip->speed_distribution, sizeof(dest_cdn_node->speed_distribution[time_step]));
			}
			else {
				...
				int m;
				for (m = 0; m < 6; m++) {
					dest_cdn_node->speed_distribution[time_step][m] += src_cdn_ip->speed_distribution[m];
				}
			}
			pthread_spin_unlock(&lock_cdn_node);
		}
