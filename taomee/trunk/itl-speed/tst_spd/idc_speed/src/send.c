 ...
 */
int update_folder(char *folder,int flag)
{
	FILE *fp;
	int year;
	int month;
	...
	sprintf(folder, "%d-%02d-%02d", year, month, day);
	if (flag == TEST_FOLDER_EXISTENCE) {
		DIR *dir;
		char buffer[FULL_PATH_LEN];
		sprintf(buffer, "%s/%s", data_store_path, folder);
		dir = opendir((const char *)buffer);
		if (dir == NULL)
			return -1; //next dir dose not exist;
		else {
			closedir(dir);
			return 0;//next dir do exist;
		}
	}
 ...
 */
int processdir(DIR *dir, char *folder, char *file)
{
	struct dirent 	*ptr;
	file_t			*mylink = NULL;
	file_t			*tmp = NULL;
	...
	else if (update_folder(tmpfolder, TEST_FOLDER_EXISTENCE) == 0) {
