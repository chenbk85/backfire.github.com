...
class c_rrd_handler
{
    public :
        c_rrd_handler();
        ~c_rrd_handler();
        int init(const char *p_rrd_dir, int grid_id);
         ...
         * @return  成功0失败-1
         */
        int write_data_to_rrd(const char *source, const char *host, const char *metric, const char *sum,
 ...
 * @return  void 
 */
inline void c_rrd_handler::my_mkdir(const char *dir)
{
