...
int c_net_client_impl::init(int server_addr, int server_port, int timeout)
{
    ...
    if(connect_to_server(m_timeout) != 0)
    {
        ...
        memcpy(&tmp_addr, &m_server_addr, 4);
        ERROR_LOG("can not connect to server(%s: %d) now.", inet_ntoa(tmp_addr), m_server_port);
...
int c_net_client_impl::ping(int timeout)
{
    if(m_sock_fd < 0)
    {
        ...
        {
            ERROR_LOG("can not connect to server");
            return -1;
        }
    }
...
int c_net_client_impl::recv_data_from_server()
{
    ...
    int bytes_recved = recv_from_server(m_sock_fd, m_recv_buffer + m_recv_buffer_len, 
            sizeof(m_recv_buffer) - m_recv_buffer_len);
    if(bytes_recved < 0)
    {
        disconnect_from_server();
    }
