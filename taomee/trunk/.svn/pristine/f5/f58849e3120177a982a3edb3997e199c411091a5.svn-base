...
extern int   *g_child_stderr_array;
...
int check_hostalive(const char *host, unsigned int timeout)
{
    ...
    child_stderr = fdopen(g_child_stderr_array[fileno(child_stdout)], "r");

    while(fgets(buf, sizeof(buf) - 1, child_stdout))
    {
        //把结尾的换行符去掉，打在日志里挺乱的
        char *endline  = NULL;
        ...
        if(sscanf(buf, "%*d packets transmitted, %*d packets received, +%*d errors, %d%% packet loss", &pl) > 0 ||
                ...
                sscanf(buf, "%*d packets transmitted %*d received, +%*d errors, %d%% packet loss", &pl) > 0) 
        { 
            DEBUG_LOG("Stdout:%s,packets loss rate:%d%s", buf, pl, "\%");
            continue;
        } 
