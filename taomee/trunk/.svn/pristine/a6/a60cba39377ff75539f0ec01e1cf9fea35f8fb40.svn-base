...
static char inside_ip[16] = {0};
static char outside_ip[16] = {0};
...
int update_server_ip(int ip_type)
{
    if (OA_IP_OUTSIDE == ip_type) {
        if (0 == get_host_ip_by_name("eth0", outside_ip)) {///一般情况，外网是eth0
            return 0;
        }
    } else {
        if (0 == get_host_ip_by_name("eth1", inside_ip)) {///一般情况，内网是eth1
            return 0;
        }
    }
...
{
    update_server_ip(OA_IP_OUTSIDE);
    *val = outside_ip;
    return 0;
}
