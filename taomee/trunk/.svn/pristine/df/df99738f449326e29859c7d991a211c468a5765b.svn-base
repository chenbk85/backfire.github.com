...
#define NET_CLIENT_MAX_SEND_BUFFER_LEN (1024 * 1024)
...
class c_net_client_impl
{
    public:
        c_net_client_impl();
        ~c_net_client_impl();
        int init(int server_addr, int server_port, int timeout);
        int send_data(char *p_data, int data_len);
        int send_data_atomic(char *p_data, int data_len);
        int recv_data(char *p_recv_buffer, int buffer_len);
        int do_io();
        int ping(int timeout = 0);
        int uninit();
        int release();
        int get_fd();
...
inline int c_net_client_impl::get_fd()
{
    return m_sock_fd;
}
#endif 
