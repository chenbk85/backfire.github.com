...
int db_mgr_get_db_base_info(int fd, const char * buf, uint32_t len)
{
    ...
    do
    {
        ...
        if (db_name[0] == 0)
        {
            GEN_SQLSTR(g_sql, "SHOW DATABASES");
        }
        else if (table_name[0] == 0)
        {
            GEN_SQLSTR(g_sql, "SHOW TABLES FROM %s", db_name);
        }
        else if (column_name[0] == 0)
        {
            GEN_SQLSTR(g_sql, "SHOW COLUMNS FROM %s.%s", db_name, table_name);
        }
        else
        {
        ...
        if(0 != g_mysql->init(DB_MGR_HOST, db_instance.port, DB_MGR_NAME, DB_MGR_USER, DB_MGR_PASS, "utf8"))
        {
...
int db_mgr_get_privilege_info(int fd, const char * buf, uint32_t len)
{
    ...
    result = get_priv(port, &priv, priv_vec);
...
int db_mgr_create_db_user(int fd, const char * buf, uint32_t len)
{
    ...
    do
    {
        if (0 != g_mysql->init(DB_MGR_HOST, db_instance.port, DB_MGR_NAME, dba_user, dba_pass, "utf8"))
        {
            result = g_mysql->get_last_errno();
            STORE_CONNECT_FAIL(buf);
            break;
    ...
    if (do_sql)
    {
        ...
        do
        {
            ...
            // {
                ...
                // db_store_db_mgr_result(g_out_common->serial_no1, 
                        // g_out_common->serial_no2, sql_no, DB_MGR_ERR_OK, g_sql);
            // }
...
int db_mgr_check_user_password(int fd, const char * buf, uint32_t len)
{
    ...
    if (do_sql)
    {
        ...
        do
        {
            if (0 != g_mysql->init(DB_MGR_HOST, db_instance.port, "mysql", DB_MGR_USER, DB_MGR_PASS, "utf8"))
            {
                // 没连上
                result = g_mysql->get_last_errno();
                STORE_CONNECT_FAIL(buf);
